...
const minerAddress = '0x83C99785E0266FB69307a93FEA79D5C70E5A29cB'; //mainnet contract   
const tokenAddress = '0x30FCB23A906493371b1721C8feb8815804808D74'; //mainnet Token
...
function buyEggs(){
	...
	if(+amt < +minDeposit) {
		alert(`you cannot deposit less than ${readableBUSD(minDeposit, 2)} sDAI`);
        return
    }
	
	var amt = web3.utils.toWei(busd);
	if(+amt + +totalDeposits > +maxDeposit) {
		alert(`you cannot deposit more than ${readableBUSD(maxDeposit, 2)} sDAI`);
        return
    }
	
    if(+amt > usrBal) {
		alert("you do not have " + busd + " sDAI in your wallet");
        return
    }
    if (+spend < +busd) {
        var amtToSpend = busd - spend;
        alert("you first need to approve " + amtToSpend + " sDAI before depositing");
        return
    }
