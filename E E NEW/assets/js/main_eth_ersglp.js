...
const minerAddress = '0xE1F6C6A4f7c22C20E9125977763433B4d0fd48eC'; //mainnet contract   
const tokenAddress = '0x574B25664508A1CAbd6f1aF3EB6386EBDe506942'; //mainnet Token 
...
function buyEggs(){
	...
	if(+amt < +minDeposit) {
		alert(`you cannot deposit less than ${readableBUSD(minDeposit, 2)} ERSg/Pls`);
        return
    }
	
	var amt = web3.utils.toWei(busd);
	if(+amt + +totalDeposits > +maxDeposit) {
		alert(`you cannot deposit more than ${readableBUSD(maxDeposit, 2)} ERSg/Pls`);
        return
    }
	
    if(+amt > usrBal) {
		alert("you do not have " + busd + " ERSg/Pls in your wallet");
        return
    }
    if (+spend < +busd) {
        var amtToSpend = busd - spend;
        alert("you first need to approve " + amtToSpend + " ERSg/Pls before depositing");
        return
    }
