...
const minerAddress = '0xe158081cbE2D78cbc879365c99CF9f6dE55f4e21'; //mainnet contract   
const tokenAddress = '0x6386704cD6f7A584EA9D23cccA66aF7EBA5a727e'; //mainnet Token
...
function buyEggs(){
	...
	if(+amt < +minDeposit) {
		alert(`you cannot deposit less than ${readableBUSD(minDeposit, 2)} Spark`);
        return
    }
	
	var amt = web3.utils.toWei(busd);
	if(+amt + +totalDeposits > +maxDeposit) {
		alert(`you cannot deposit more than ${readableBUSD(maxDeposit, 2)} Spark`);
        return
    }
	
    if(+amt > usrBal) {
		alert("you do not have " + busd + " Spark in your wallet");
        return
    }
    if (+spend < +busd) {
        var amtToSpend = busd - spend;
        alert("you first need to approve " + amtToSpend + " Spark before depositing");
        return
    }
