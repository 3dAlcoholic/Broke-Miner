...
const minerAddress = '0xA7eB1a74f00FfBeddfF5015F7a34481881Bb9C2B'; //mainnet contract   
const tokenAddress = '0x6B175474E89094C44Da98b954EedeAC495271d0F'; //mainnet Token
...
function buyEggs(){
	...
	if(+amt < +minDeposit) {
		alert(`you cannot deposit less than ${readableBUSD(minDeposit, 2)} pDai`);
        return
    }
	
	var amt = web3.utils.toWei(busd);
	if(+amt + +totalDeposits > +maxDeposit) {
		alert(`you cannot deposit more than ${readableBUSD(maxDeposit, 2)} pDai`);
        return
    }
	
    if(+amt > usrBal) {
		alert("you do not have " + busd + " pDai in your wallet");
        return
    }
    if (+spend < +busd) {
        var amtToSpend = busd - spend;
        alert("you first need to approve " + amtToSpend + " pDai before depositing");
        return
    }
