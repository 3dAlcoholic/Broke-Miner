...
const minerAddress = '0x766DA9aac70616AfA1fb4Bd0Ec905475F85F1dE0'; //mainnet contract   
const tokenAddress = '0x80A2fF0E7768AF877FB4e9D57f27CFC4A0865Ddc'; //mainnet Token
...
function buyEggs(){
	...
	if(+amt < +minDeposit) {
		alert(`you cannot deposit less than ${readableBUSD(minDeposit, 2)} ERSp/Pls`);
        return
    }
	
	var amt = web3.utils.toWei(busd);
	if(+amt + +totalDeposits > +maxDeposit) {
		alert(`you cannot deposit more than ${readableBUSD(maxDeposit, 2)} ERSp/Pls`);
        return
    }
	
    if(+amt > usrBal) {
		alert("you do not have " + busd + " ERSp/Pls in your wallet");
        return
    }
    if (+spend < +busd) {
        var amtToSpend = busd - spend;
        alert("you first need to approve " + amtToSpend + " ERSp/Pls before depositing");
        return
    }
