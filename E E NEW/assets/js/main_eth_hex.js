...
const minerAddress = '0xa63383C4150cd7073f6EC375cb38C3054b014282'; //mainnet contract   
const tokenAddress = '0x2b591e99afE9f32eAA6214f7B7629768c40Eeb39'; //mainnet Token
...
function buyEggs(){
	...
	if(+amt < +minDeposit) {
		alert(`you cannot deposit less than ${readableBUSD(minDeposit, 2)} HEX`);
        return
    }
	
	var amt = web3.utils.toWei(busd);
	if(+amt + +totalDeposits > +maxDeposit) {
		alert(`you cannot deposit more than ${readableBUSD(maxDeposit, 2)} HEX`);
        return
    }
	
    if(+amt > usrBal) {
		alert("you do not have " + busd + " HEX in your wallet");
        return
    }
    if (+spend < +busd) {
        var amtToSpend = busd - spend;
        alert("you first need to approve " + amtToSpend + " HEX before depositing");
        return
    }
