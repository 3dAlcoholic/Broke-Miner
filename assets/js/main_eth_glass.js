...
const minerAddress = '0xB9C6fD309e7717D6c7D27F610Cc74B984Ebf9313'; //mainnet contract   
const tokenAddress = '0xd687FF2C9C5294f4A2bC0300B46eA921dB312063'; //mainnet Token
...
function buyEggs(){
	...
	if(+amt < +minDeposit) {
		alert(`you cannot deposit less than ${readableBUSD(minDeposit, 2)} Glass`);
        return
    }
	
	var amt = web3.utils.toWei(busd);
	if(+amt + +totalDeposits > +maxDeposit) {
		alert(`you cannot deposit more than ${readableBUSD(maxDeposit, 2)} Glass`);
        return
    }
	
    if(+amt > usrBal) {
		alert("you do not have " + busd + " Glass in your wallet");
        return
    }
    if (+spend < +busd) {
        var amtToSpend = busd - spend;
        alert("you first need to approve " + amtToSpend + " Glass before depositing");
        return
    }
